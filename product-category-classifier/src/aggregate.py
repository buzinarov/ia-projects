...
def load_seed_metrics(model_name, seeds, tag=""):
    return [json.loads((ARTIFACTS_DIR / f"metrics_{model_name}{tag}_seed{s}.json").read_text()) for s in seeds]
...
def aggregate_model(model_name, seeds, tag=""):
    runs = load_seed_metrics(model_name, seeds, tag=tag)
    ...
    plot_confusion_matrix(
        ...
        ARTIFACTS_DIR / f"confusion_matrix_{model_name}{tag}_summary.png",
        f"{model_name}{tag} - confusion matrix (summed across seeds {seeds})",
    )
    return summary
