"""Train either the baseline (image-only) or proposed (image + gender)
model. Both are trained identically (same split, same epochs, same loss)
so the only real difference between them is whether the model sees the
second modality.
...
def run_epoch(model, loader, criterion, optimizer, device, train):
    ...
    with torch.set_grad_enabled(train):
        for (images, genders), labels in loader:
            images, genders, labels = images.to(device), genders.to(device), labels.to(device)
            if train:
                optimizer.zero_grad()
...
def main():
    ...
    train_loader, val_loader, _, maps = get_dataloaders(
        batch_size=args.batch_size, subset_frac=args.subset_frac
    )
    num_classes = len(maps["masterCategory_classes"])
    num_genders = len(maps["gender_classes"])

    model = build_model(args.model, num_classes=num_classes, num_genders=num_genders, img_size=IMG_SIZE)
    model.to(device)
    ...
    (ARTIFACTS_DIR / f"history_{args.model}.json").write_text(json.dumps(history, indent=2))
    print(f"Saved checkpoint and history for '{args.model}'")
