...
def test_build_record_has_required_fields():
    record = build_prediction_record("Topwear", 0.9, "proposed", product_id="p1")
    for field in REQUIRED_FIELDS:
        assert field in record


def test_build_record_defaults_product_id_when_missing():
    record = build_prediction_record("Topwear", 0.9, "proposed")
    assert record["product_id"].startswith("upload-")


def test_validate_accepts_well_formed_record(label_maps):
    record = build_prediction_record(label_maps["target_classes"][0], 0.9, "proposed", product_id="p1")
...
def test_validate_rejects_unknown_class(label_maps):
    record = build_prediction_record("NotARealSubCategory", 0.9, "proposed", product_id="p1")
...
def test_validate_rejects_out_of_bounds_confidence(label_maps):
    record = build_prediction_record(label_maps["target_classes"][0], 1.5, "proposed", product_id="p1")
    with pytest.raises(ValueError):
        validate_prediction_record(record, label_maps["target_classes"])
...
def test_validate_rejects_unknown_model_name(label_maps):
    record = build_prediction_record(label_maps["target_classes"][0], 0.9, "not-a-real-model", product_id="p1")
    with pytest.raises(ValueError):
        validate_prediction_record(record, label_maps["target_classes"])
